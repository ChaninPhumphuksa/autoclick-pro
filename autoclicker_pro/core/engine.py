...
class AutoClicker:
    ...
    def start(self):
        ...
        self.status_cb("Auto Click: กำลังทำงาน...")
    ...
    def stop(self):
        ...
        self.status_cb("Auto Click: หยุดแล้ว")
    ...
    def _run(self):
        ...
        self.status_cb(f"Auto Click: หยุดแล้ว (คลิกไปทั้งหมด {count} ครั้ง)")
...
class MacroRecorder:
    ...
    def start(self):
        ...
        self.status_cb("กำลังอัดมาโคร... (บันทึกแล้ว 0 เหตุการณ์)")
        self.on_change()
    ...
    def stop(self):
        ...
        self.status_cb(f"หยุดอัดมาโคร (บันทึกได้ {len(self.events)} เหตุการณ์)")
        self.on_change()
    ...
    def _on_click(self, x, y, button, pressed):
        ...
        self.status_cb(f"กำลังอัดมาโคร... (บันทึกแล้ว {len(self.events)} เหตุการณ์)")
        self.on_change()
...
class MacroPlayer:
    ...
    def start(self):
        ...
        if self.running or pyautogui is None:
            return
        if not events:
            self.status_cb("ไม่มีมาโครให้เล่น กรุณาอัดหรือสร้างมาโครก่อน")
            return
        self._stop_event.clear()
        self.running = True
        self._thread = threading.Thread(target=self._run, daemon=True)
    ...
    def stop(self):
        ...
        self.status_cb("เล่นมาโคร: หยุดแล้ว")
    ...
    def _run(self):
        ...
        self.status_cb(f"เล่นมาโคร: หยุดแล้ว (เล่นไปทั้งหมด {runs} รอบ)")
