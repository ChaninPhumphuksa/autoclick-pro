# -*- coding: utf-8 -*-
"""
gui/dialogs.py
==============
กล่องโต้ตอบสำหรับเพิ่ม/แก้ไขขั้นตอนของชุดคำสั่งทีละรายการ ใช้ภาษาที่เข้าใจง่าย
ไม่ใช้ศัพท์เทคนิค
"""

import tkinter as tk
from tkinter import ttk, messagebox, simpledialog

from .fonts import ui_font
from .capture_hub import capture_hub
from core.positions import load_positions, add_or_update_position, delete_position
from core import windows_target

try:
    import pyautogui
except ImportError:
    pyautogui = None

try:
    from pynput import keyboard as pynput_keyboard
except ImportError:
    pynput_keyboard = None

# แปลชื่อปุ่มพิเศษเป็นภาษาไทยให้ผู้ใช้เห็น แต่ค่าที่เก็บจริงยังเป็นชื่อปุ่มมาตรฐาน
SPECIAL_KEY_LABELS = {
    "": "— ไม่เลือก —",
    "space": "เว้นวรรค (Space)",
    "enter": "ตอบตกลง (Enter)",
    "tab": "แท็บ (Tab)",
    "backspace": "ลบถอยหลัง (Backspace)",
    "up": "ลูกศรขึ้น",
    "down": "ลูกศรลง",
    "left": "ลูกศรซ้าย",
    "right": "ลูกศรขวา",
    "shift": "Shift",
    "ctrl": "Ctrl",
    "alt": "Alt",
}
SPECIAL_KEY_VALUES = {v: k for k, v in SPECIAL_KEY_LABELS.items()}


class StepDialog(tk.Toplevel):
    def __init__(self, parent, step_type, existing=None, app_ref=None):
        super().__init__(parent)
        self.result = None
        self.step_type = step_type
        self.app_ref = app_ref
        titles = {"click": "เพิ่มขั้นตอน: คลิกที่ตำแหน่งนี้",
                  "drag": "เพิ่มขั้นตอน: ลากเมาส์",
                  "key": "เพิ่มขั้นตอน: กดปุ่มนี้",
                  "type": "เพิ่มขั้นตอน: พิมพ์ข้อความ",
                  "wait": "เพิ่มขั้นตอน: รอสักครู่",
                  "wait_pixel": "เพิ่มขั้นตอน: รอจนกว่าสีจะเปลี่ยน",
                  "loop_start": "เพิ่มขั้นตอน: เริ่มวนซ้ำ",
                  "loop_end": "เพิ่มขั้นตอน: จบการวนซ้ำ"}
        self.title(titles[step_type])
        self.resizable(False, False)
        self.transient(parent)
        self.grab_set()
        self.configure(padx=4, pady=4)

        bg_enabled = False
        if self.app_ref is not None:
            try:
                bg_enabled, bg_title, _ = self.app_ref.get_background_target()
            except Exception:
                bg_enabled, bg_title = False, None
        if bg_enabled and step_type in ("click", "drag"):
            ttk.Label(self, text=f"🖥 โหมดเบื้องหลังเปิดอยู่ — ตำแหน่งที่จับจะอ้างอิงกับ \"{bg_title}\"",
                      font=ui_font(9), foreground="#F59E0B", wraplength=380, justify="left").grid(
                row=0, column=0, columnspan=2, sticky="w", padx=12, pady=(8, 0))
            row_offset = 1
        else:
            row_offset = 0

        existing = existing or {}
        pad = {"padx": 12, "pady": 8}
        row = row_offset

        ttk.Label(self, text="รอกี่วินาทีก่อนทำขั้นตอนนี้:", font=ui_font(10)).grid(
            row=row, column=0, sticky="w", **pad)
        self.delay_var = tk.StringVar(value=str(existing.get("delay", 0.5)))
        ttk.Entry(self, textvariable=self.delay_var, width=10, font=ui_font(10)).grid(
            row=row, column=1, **pad)
        row += 1

        if step_type == "click":
            ttk.Label(self, text="ตำแหน่งแนวนอน (X):", font=ui_font(10)).grid(
                row=row, column=0, sticky="w", **pad)
            self.x_var = tk.StringVar(value=str(existing.get("x", 0)))
            ttk.Entry(self, textvariable=self.x_var, width=10, font=ui_font(10)).grid(
                row=row, column=1, **pad)
            row += 1

            ttk.Label(self, text="ตำแหน่งแนวตั้ง (Y):", font=ui_font(10)).grid(
                row=row, column=0, sticky="w", **pad)
            self.y_var = tk.StringVar(value=str(existing.get("y", 0)))
            ttk.Entry(self, textvariable=self.y_var, width=10, font=ui_font(10)).grid(
                row=row, column=1, **pad)
            row += 1

            ttk.Label(self, text="ปุ่มเมาส์:", font=ui_font(10)).grid(row=row, column=0, sticky="w", **pad)
            self.button_var = tk.StringVar(value=existing.get("button", "left"))
            btn_frame = ttk.Frame(self)
            btn_frame.grid(row=row, column=1, sticky="w")
            for text, val in [("ซ้าย", "left"), ("ขวา", "right"), ("กลาง", "middle")]:
                ttk.Radiobutton(btn_frame, text=text, value=val, variable=self.button_var).pack(side="left")
            row += 1

            self.capture_status = tk.StringVar(value="")
            capture_row = ttk.Frame(self)
            capture_row.grid(row=row, column=0, columnspan=2, **pad)
            ttk.Button(capture_row, text="📍 จิ้มตำแหน่งจากเมาส์ (3 วิ)",
                       command=self._start_capture).pack(side="left", padx=(0, 6))
            ttk.Button(capture_row, text="🗂 เลือกจากที่บันทึกไว้",
                       command=self._open_position_picker).pack(side="left")
            row += 1
            ttk.Label(self, textvariable=self.capture_status, foreground="gray", font=ui_font(9)).grid(
                row=row, column=0, columnspan=2)
            row += 1

        elif step_type == "drag":
            ttk.Label(self, text="จุดเริ่มลาก:", font=ui_font(10, "bold")).grid(
                row=row, column=0, sticky="w", **pad)
            row += 1
            ttk.Label(self, text="X1:", font=ui_font(10)).grid(row=row, column=0, sticky="w", **pad)
            self.x1_var = tk.StringVar(value=str(existing.get("x1", 0)))
            ttk.Entry(self, textvariable=self.x1_var, width=10, font=ui_font(10)).grid(
                row=row, column=1, **pad)
            row += 1
            ttk.Label(self, text="Y1:", font=ui_font(10)).grid(row=row, column=0, sticky="w", **pad)
            self.y1_var = tk.StringVar(value=str(existing.get("y1", 0)))
            ttk.Entry(self, textvariable=self.y1_var, width=10, font=ui_font(10)).grid(
                row=row, column=1, **pad)
            row += 1
            start_row = ttk.Frame(self)
            start_row.grid(row=row, column=0, columnspan=2, **pad)
            ttk.Button(start_row, text="📍 จิ้มจุดเริ่ม (3 วิ)",
                       command=lambda: self._capture_point(self.x1_var, self.y1_var)).pack(side="left", padx=(0, 6))
            ttk.Button(start_row, text="🗂 เลือกจากที่บันทึกไว้",
                       command=lambda: self._pick_point(self.x1_var, self.y1_var)).pack(side="left")
            row += 1

            ttk.Separator(self).grid(row=row, column=0, columnspan=2, sticky="ew", pady=4)
            row += 1

            ttk.Label(self, text="จุดปล่อยลาก:", font=ui_font(10, "bold")).grid(
                row=row, column=0, sticky="w", **pad)
            row += 1
            ttk.Label(self, text="X2:", font=ui_font(10)).grid(row=row, column=0, sticky="w", **pad)
            self.x2_var = tk.StringVar(value=str(existing.get("x2", 0)))
            ttk.Entry(self, textvariable=self.x2_var, width=10, font=ui_font(10)).grid(
                row=row, column=1, **pad)
            row += 1
            ttk.Label(self, text="Y2:", font=ui_font(10)).grid(row=row, column=0, sticky="w", **pad)
            self.y2_var = tk.StringVar(value=str(existing.get("y2", 0)))
            ttk.Entry(self, textvariable=self.y2_var, width=10, font=ui_font(10)).grid(
                row=row, column=1, **pad)
            row += 1
            end_row = ttk.Frame(self)
            end_row.grid(row=row, column=0, columnspan=2, **pad)
            ttk.Button(end_row, text="📍 จิ้มจุดปล่อย (3 วิ)",
                       command=lambda: self._capture_point(self.x2_var, self.y2_var)).pack(side="left", padx=(0, 6))
            ttk.Button(end_row, text="🗂 เลือกจากที่บันทึกไว้",
                       command=lambda: self._pick_point(self.x2_var, self.y2_var)).pack(side="left")
            row += 1

            ttk.Label(self, text="ปุ่มเมาส์:", font=ui_font(10)).grid(row=row, column=0, sticky="w", **pad)
            self.button_var = tk.StringVar(value=existing.get("button", "left"))
            drag_btn_frame = ttk.Frame(self)
            drag_btn_frame.grid(row=row, column=1, sticky="w")
            for text, val in [("ซ้าย", "left"), ("ขวา", "right"), ("กลาง", "middle")]:
                ttk.Radiobutton(drag_btn_frame, text=text, value=val, variable=self.button_var).pack(side="left")
            row += 1

            ttk.Label(self, text="ลากนานกี่วินาที:", font=ui_font(10)).grid(
                row=row, column=0, sticky="w", **pad)
            self.duration_var = tk.StringVar(value=str(existing.get("duration", 0.3)))
            ttk.Entry(self, textvariable=self.duration_var, width=10, font=ui_font(10)).grid(
                row=row, column=1, **pad)
            row += 1

            self.capture_status = tk.StringVar(value="")
            ttk.Label(self, textvariable=self.capture_status, foreground="gray", font=ui_font(9)).grid(
                row=row, column=0, columnspan=2)
            row += 1

        elif step_type == "key":
            existing_key = existing.get("key", "a")
            existing_is_special = existing_key in SPECIAL_KEY_LABELS and existing_key != ""

            ttk.Label(self, text="พิมพ์ตัวอักษรที่ต้องการกด (เช่น a, 1):", font=ui_font(10)).grid(
                row=row, column=0, sticky="w", **pad)
            self.key_var = tk.StringVar(value="" if existing_is_special else existing_key)
            ttk.Entry(self, textvariable=self.key_var, width=10, font=ui_font(10)).grid(
                row=row, column=1, **pad)
            row += 1

            ttk.Label(self, text="หรือเลือกปุ่มพิเศษ:", font=ui_font(10)).grid(
                row=row, column=0, sticky="w", **pad)
            initial_label = SPECIAL_KEY_LABELS.get(existing_key, SPECIAL_KEY_LABELS[""]) if existing_is_special else SPECIAL_KEY_LABELS[""]
            self.special_display_var = tk.StringVar(value=initial_label)
            ttk.Combobox(self, textvariable=self.special_display_var,
                         values=list(SPECIAL_KEY_LABELS.values()),
                         state="readonly", width=18, font=ui_font(10)).grid(row=row, column=1, **pad)
            row += 1

            ttk.Label(self, text="กดพร้อมปุ่มเสริม (ถ้ามี):", font=ui_font(10)).grid(
                row=row, column=0, sticky="w", **pad)
            existing_mods = set(existing.get("modifiers", []))
            self.mod_ctrl_var = tk.BooleanVar(value="ctrl" in existing_mods)
            self.mod_alt_var = tk.BooleanVar(value="alt" in existing_mods)
            self.mod_shift_var = tk.BooleanVar(value="shift" in existing_mods)
            mod_frame = ttk.Frame(self)
            mod_frame.grid(row=row, column=1, sticky="w")
            ttk.Checkbutton(mod_frame, text="Ctrl", variable=self.mod_ctrl_var).pack(side="left")
            ttk.Checkbutton(mod_frame, text="Alt", variable=self.mod_alt_var).pack(side="left", padx=(8, 0))
            ttk.Checkbutton(mod_frame, text="Shift", variable=self.mod_shift_var).pack(side="left", padx=(8, 0))
            row += 1
            ttk.Label(self, text="เช่น ติ๊ก Ctrl แล้วพิมพ์ c = กด Ctrl+C",
                      font=ui_font(9), foreground="gray").grid(row=row, column=0, columnspan=2, sticky="w", padx=12)
            row += 1

        elif step_type == "type":
            ttk.Label(self, text="ข้อความที่จะพิมพ์:", font=ui_font(10)).grid(
                row=row, column=0, sticky="w", **pad)
            self.text_var = tk.StringVar(value=existing.get("text", ""))
            ttk.Entry(self, textvariable=self.text_var, width=30, font=ui_font(10)).grid(
                row=row, column=1, **pad)
            row += 1
            ttk.Label(self, text="พิมพ์ทั้งข้อความในครั้งเดียว (รองรับภาษาไทย/อังกฤษ)",
                      font=ui_font(9), foreground="gray").grid(row=row, column=0, columnspan=2, sticky="w", padx=12)
            row += 1

        elif step_type == "wait_pixel":
            ttk.Label(self, text="ตำแหน่งที่จะตรวจสอบสี (ใช้พิกัดหน้าจอจริงเสมอ ไม่รองรับโหมดเบื้องหลัง):",
                      font=ui_font(10, "bold"), wraplength=280, justify="left").grid(
                row=row, column=0, columnspan=2, sticky="w", **pad)
            row += 1
            ttk.Label(self, text="X:", font=ui_font(10)).grid(row=row, column=0, sticky="w", **pad)
            self.px_var = tk.StringVar(value=str(existing.get("x", 0)))
            ttk.Entry(self, textvariable=self.px_var, width=10, font=ui_font(10)).grid(row=row, column=1, **pad)
            row += 1
            ttk.Label(self, text="Y:", font=ui_font(10)).grid(row=row, column=0, sticky="w", **pad)
            self.py_var = tk.StringVar(value=str(existing.get("y", 0)))
            ttk.Entry(self, textvariable=self.py_var, width=10, font=ui_font(10)).grid(row=row, column=1, **pad)
            row += 1

            existing_color = existing.get("color", [0, 0, 0])
            self.color_var = tk.StringVar(value=str(tuple(existing_color)))
            color_row = ttk.Frame(self)
            color_row.grid(row=row, column=0, columnspan=2, **pad)
            ttk.Label(color_row, text="สีเป้าหมาย (R,G,B): ").pack(side="left")
            ttk.Entry(color_row, textvariable=self.color_var, width=14, font=ui_font(10)).pack(side="left")
            self.color_swatch = tk.Label(color_row, width=3, bg="#%02x%02x%02x" % tuple(existing_color))
            self.color_swatch.pack(side="left", padx=6)
            ttk.Button(color_row, text="🎯 จับสีจากตำแหน่งเมาส์ (3 วิ)",
                       command=self._capture_pixel_color).pack(side="left", padx=6)
            row += 1

            self.match_var = tk.BooleanVar(value=existing.get("match", True))
            ttk.Radiobutton(self, text="รอจนกว่าสี ณ ตำแหน่งนี้ จะ \"เป็น\" สีเป้าหมาย", value=True,
                             variable=self.match_var).grid(row=row, column=0, columnspan=2, sticky="w", padx=12)
            row += 1
            ttk.Radiobutton(self, text="รอจนกว่าสี ณ ตำแหน่งนี้ จะ \"ไม่เป็น\" สีเป้าหมาย", value=False,
                             variable=self.match_var).grid(row=row, column=0, columnspan=2, sticky="w", padx=12)
            row += 1

            ttk.Label(self, text="รอนานสุดกี่วินาที (ถ้าเกินจะข้ามขั้นตอนนี้ไปเลย):", font=ui_font(10)).grid(
                row=row, column=0, sticky="w", **pad)
            self.timeout_var = tk.StringVar(value=str(existing.get("timeout", 5)))
            ttk.Entry(self, textvariable=self.timeout_var, width=10, font=ui_font(10)).grid(
                row=row, column=1, **pad)
            row += 1
            self.pixel_status_var = tk.StringVar(value="")
            ttk.Label(self, textvariable=self.pixel_status_var, foreground="gray", font=ui_font(9)).grid(
                row=row, column=0, columnspan=2)
            row += 1

        elif step_type == "loop_start":
            ttk.Label(self, text="วนซ้ำขั้นตอนที่อยู่ระหว่างนี้กับ \"จบการวนซ้ำ\" กี่รอบ:",
                      font=ui_font(10), wraplength=280, justify="left").grid(
                row=row, column=0, sticky="w", **pad)
            self.repeat_var = tk.StringVar(value=str(existing.get("repeat", 2)))
            ttk.Entry(self, textvariable=self.repeat_var, width=10, font=ui_font(10)).grid(
                row=row, column=1, **pad)
            row += 1
            ttk.Label(self, text="อย่าลืมเพิ่มขั้นตอน \"จบการวนซ้ำ\" ปิดท้ายด้วย",
                      font=ui_font(9), foreground="gray").grid(row=row, column=0, columnspan=2, sticky="w", padx=12)
            row += 1

        elif step_type == "loop_end":
            ttk.Label(self, text="เป็นจุดสิ้นสุดของการวนซ้ำที่ใกล้ที่สุดก่อนหน้านี้ ไม่ต้องตั้งค่าอะไรเพิ่ม",
                      font=ui_font(10), wraplength=280, justify="left").grid(
                row=row, column=0, columnspan=2, sticky="w", padx=12, pady=8)
            row += 1

        ttk.Separator(self).grid(row=row, column=0, columnspan=2, sticky="ew", pady=8)
        row += 1
        btns = ttk.Frame(self)
        btns.grid(row=row, column=0, columnspan=2, pady=10)
        ttk.Button(btns, text="บันทึก", style="Accent.TButton", command=self._on_save).pack(side="left", padx=6)
        ttk.Button(btns, text="ยกเลิก", command=self.destroy).pack(side="left", padx=6)

    def _resolve_capture_point(self):
        """คืนตำแหน่งเมาส์ปัจจุบัน แปลงเป็นพิกัดสัมพัทธ์กับหน้าต่างเป้าหมายถ้าเปิดโหมดเบื้องหลังอยู่"""
        x, y = pyautogui.position()
        if self.app_ref is not None:
            try:
                enabled, title, process = self.app_ref.get_background_target()
            except Exception:
                enabled, title, process = False, None, None
            if enabled:
                hwnd = windows_target.resolve_window(title, process)
                if hwnd is not None:
                    x, y = windows_target.screen_to_client(hwnd, x, y)
        return x, y

    def _start_capture(self):
        def do_capture():
            if pyautogui:
                x, y = self._resolve_capture_point()
                self.x_var.set(str(x))
                self.y_var.set(str(y))
                self.capture_status.set(f"จิ้มตำแหน่งแล้ว ✓ ({x}, {y})")
            capture_hub.clear(do_capture)

        capture_hub.set_pending(do_capture)

        def countdown(n):
            if not capture_hub.is_pending(do_capture):
                return
            if n > 0:
                self.capture_status.set(f"เตรียมตัว... อีก {n} วินาที (หรือกดปุ่มลัดจับตำแหน่งทันทีที่พร้อม)")
                self.after(1000, lambda: countdown(n - 1))
            else:
                do_capture()
        countdown(3)

    def _open_position_picker(self):
        dialog = PositionPickerDialog(self)
        self.wait_window(dialog)
        if dialog.result:
            x, y = dialog.result
            self.x_var.set(str(x))
            self.y_var.set(str(y))
            self.capture_status.set(f"เลือกตำแหน่งแล้ว ✓ ({x}, {y})")

    def _capture_point(self, x_var, y_var):
        def do_capture():
            if pyautogui:
                x, y = self._resolve_capture_point()
                x_var.set(str(x))
                y_var.set(str(y))
                self.capture_status.set(f"จิ้มตำแหน่งแล้ว ✓ ({x}, {y})")
            capture_hub.clear(do_capture)

        capture_hub.set_pending(do_capture)

        def countdown(n):
            if not capture_hub.is_pending(do_capture):
                return
            if n > 0:
                self.capture_status.set(f"เตรียมตัว... อีก {n} วินาที (หรือกดปุ่มลัดจับตำแหน่งทันทีที่พร้อม)")
                self.after(1000, lambda: countdown(n - 1))
            else:
                do_capture()
        countdown(3)

    def _pick_point(self, x_var, y_var):
        dialog = PositionPickerDialog(self)
        self.wait_window(dialog)
        if dialog.result:
            x, y = dialog.result
            x_var.set(str(x))
            y_var.set(str(y))
            self.capture_status.set(f"เลือกตำแหน่งแล้ว ✓ ({x}, {y})")

    def _capture_pixel_color(self):
        def do_capture():
            if pyautogui:
                # หมายเหตุ: การดูสีบนจอต้องใช้พิกัดหน้าจอจริงเสมอ (ไม่แปลงเป็นพิกัดสัมพัทธ์กับ
                # หน้าต่างเป้าหมายเหมือนปุ่มคลิก/ลาก เพราะฟีเจอร์นี้ยังไม่รองรับโหมดเบื้องหลัง)
                x, y = pyautogui.position()
                color = pyautogui.pixel(x, y)
                self.color_var.set(str(tuple(color)))
                self.color_swatch.config(bg="#%02x%02x%02x" % tuple(color))
                self.pixel_status_var.set(f"จับสีแล้ว ✓ ที่ตำแหน่ง ({x}, {y})")
                self.px_var.set(str(x))
                self.py_var.set(str(y))
            capture_hub.clear(do_capture)

        capture_hub.set_pending(do_capture)

        def countdown(n):
            if not capture_hub.is_pending(do_capture):
                return
            if n > 0:
                self.pixel_status_var.set(f"เตรียมตัว... อีก {n} วินาที (หรือกดปุ่มลัดจับตำแหน่งทันทีที่พร้อม)")
                self.after(1000, lambda: countdown(n - 1))
            else:
                do_capture()
        countdown(3)

    def _on_save(self):
        try:
            delay = float(self.delay_var.get())
        except ValueError:
            messagebox.showerror("กรอกไม่ถูกต้อง", "ช่องเวลารอ ต้องเป็นตัวเลขเท่านั้น")
            return

        if self.step_type == "click":
            try:
                x = int(float(self.x_var.get()))
                y = int(float(self.y_var.get()))
            except ValueError:
                messagebox.showerror("กรอกไม่ถูกต้อง", "ตำแหน่ง X และ Y ต้องเป็นตัวเลขเท่านั้น")
                return
            self.result = {"type": "click", "x": x, "y": y, "button": self.button_var.get(), "delay": delay}
        elif self.step_type == "drag":
            try:
                x1 = int(float(self.x1_var.get()))
                y1 = int(float(self.y1_var.get()))
                x2 = int(float(self.x2_var.get()))
                y2 = int(float(self.y2_var.get()))
                duration = max(0.05, float(self.duration_var.get()))
            except ValueError:
                messagebox.showerror("กรอกไม่ถูกต้อง", "ตำแหน่งและระยะเวลาต้องเป็นตัวเลขเท่านั้น")
                return
            self.result = {"type": "drag", "x1": x1, "y1": y1, "x2": x2, "y2": y2,
                            "button": self.button_var.get(), "duration": duration, "delay": delay}
        elif self.step_type == "key":
            special_val = SPECIAL_KEY_VALUES.get(self.special_display_var.get(), "")
            key_val = special_val or self.key_var.get().strip()
            if not key_val:
                messagebox.showerror("กรอกไม่ถูกต้อง", "กรุณาระบุปุ่มที่ต้องการกด")
                return
            modifiers = []
            if self.mod_ctrl_var.get():
                modifiers.append("ctrl")
            if self.mod_alt_var.get():
                modifiers.append("alt")
            if self.mod_shift_var.get():
                modifiers.append("shift")
            self.result = {"type": "key", "key": key_val, "modifiers": modifiers, "delay": delay}
        elif self.step_type == "type":
            text = self.text_var.get()
            if not text:
                messagebox.showerror("กรอกไม่ถูกต้อง", "กรุณาพิมพ์ข้อความที่ต้องการ")
                return
            self.result = {"type": "type", "text": text, "delay": delay}
        elif self.step_type == "wait_pixel":
            try:
                px = int(float(self.px_var.get()))
                py = int(float(self.py_var.get()))
                timeout = max(0.1, float(self.timeout_var.get()))
                color = tuple(int(v.strip()) for v in self.color_var.get().strip("() ").split(","))
                if len(color) != 3:
                    raise ValueError
            except ValueError:
                messagebox.showerror("กรอกไม่ถูกต้อง",
                                      "ตำแหน่ง, เวลา และสี (ต้องเป็น R,G,B เช่น 255,0,0) ต้องถูกต้อง")
                return
            self.result = {"type": "wait_pixel", "x": px, "y": py, "color": list(color),
                            "match": self.match_var.get(), "timeout": timeout, "delay": delay}
        elif self.step_type == "loop_start":
            try:
                repeat = max(1, int(float(self.repeat_var.get())))
            except ValueError:
                messagebox.showerror("กรอกไม่ถูกต้อง", "จำนวนรอบต้องเป็นตัวเลขเท่านั้น")
                return
            self.result = {"type": "loop_start", "repeat": repeat, "delay": delay}
        elif self.step_type == "loop_end":
            self.result = {"type": "loop_end", "delay": 0}
        else:
            self.result = {"type": "wait", "delay": delay}
        self.destroy()


class PositionPickerDialog(tk.Toplevel):
    """
    ตัวช่วยจับพิกัด (Position Picker)
    ให้ผู้ใช้บันทึกตำแหน่งบนหน้าจอไว้เป็นชื่อที่จำง่าย แล้วเรียกใช้ซ้ำได้
    ทั้งจากหน้า Auto Click และตอนสร้างขั้นตอนคลิกในชุดคำสั่ง
    """

    def __init__(self, parent):
        super().__init__(parent)
        self.result = None  # จะถูกตั้งเป็น (x, y) เมื่อผู้ใช้กด "ใช้ตำแหน่งนี้"
        self.title("🗂 ตัวช่วยจับพิกัด")
        self.resizable(False, False)
        self.transient(parent)
        self.grab_set()
        self.geometry("420x380")

        ttk.Label(self, text="บันทึกตำแหน่งบนหน้าจอไว้เป็นชื่อที่จำง่าย แล้วเลือกใช้ซ้ำได้ทุกที่ในโปรแกรม",
                  font=ui_font(9), foreground="gray", wraplength=380, justify="left").pack(
            padx=14, pady=(14, 8), anchor="w")

        columns = ("name", "x", "y")
        self.tree = ttk.Treeview(self, columns=columns, show="headings", height=8)
        self.tree.heading("name", text="ชื่อตำแหน่ง")
        self.tree.heading("x", text="X")
        self.tree.heading("y", text="Y")
        self.tree.column("name", width=220, anchor="w")
        self.tree.column("x", width=60, anchor="center")
        self.tree.column("y", width=60, anchor="center")
        self.tree.pack(padx=14, pady=4, fill="both", expand=True)

        btn_row1 = ttk.Frame(self)
        btn_row1.pack(padx=14, pady=6, fill="x")
        ttk.Button(btn_row1, text="➕ จับตำแหน่งใหม่", command=self._capture_new).pack(side="left", padx=(0, 6))
        ttk.Button(btn_row1, text="ลบ", command=self._delete_selected).pack(side="left")

        btn_row2 = ttk.Frame(self)
        btn_row2.pack(padx=14, pady=(4, 14), fill="x")
        ttk.Button(btn_row2, text="✓ ใช้ตำแหน่งนี้", style="Accent.TButton",
                   command=self._use_selected).pack(side="left", padx=(0, 6))
        ttk.Button(btn_row2, text="ปิด", command=self.destroy).pack(side="left")

        self._refresh_list()

    def _refresh_list(self):
        self.tree.delete(*self.tree.get_children())
        positions = load_positions()
        for name, coord in sorted(positions.items()):
            self.tree.insert("", "end", iid=name, values=(name, coord.get("x", 0), coord.get("y", 0)))

    def _capture_new(self):
        if pyautogui is None:
            messagebox.showinfo("ยังใช้งานไม่ได้", "ฟีเจอร์นี้ต้องติดตั้งตัวช่วยเพิ่มเติมก่อน")
            return

        status_win = tk.Toplevel(self)
        status_win.title("กำลังจับตำแหน่ง")
        status_win.resizable(False, False)
        status_win.transient(self)
        status_win.grab_set()
        status_var = tk.StringVar(value="เตรียมตัว...")
        ttk.Label(status_win, textvariable=status_var, font=ui_font(11), padding=20).pack()

        def do_capture():
            if not status_win.winfo_exists():
                return
            x, y = pyautogui.position()
            status_win.destroy()
            self._prompt_name_and_save(x, y)
            capture_hub.clear(do_capture)

        capture_hub.set_pending(do_capture)
        status_win.protocol("WM_DELETE_WINDOW", lambda: (capture_hub.clear(do_capture), status_win.destroy()))

        def countdown(n):
            if not capture_hub.is_pending(do_capture):
                return
            if n > 0:
                status_var.set(f"ย้ายเมาส์ไปตำแหน่งที่ต้องการ... อีก {n} วินาที (หรือกดปุ่มลัดจับตำแหน่งทันทีที่พร้อม)")
                status_win.after(1000, lambda: countdown(n - 1))
            else:
                do_capture()

        countdown(3)

    def _prompt_name_and_save(self, x, y):
        name = simpledialog.askstring(
            "ตั้งชื่อตำแหน่ง",
            f"จับตำแหน่งได้แล้ว: ({x}, {y})\nตั้งชื่อตำแหน่งนี้ (เช่น ปุ่มยืนยัน):",
            parent=self,
        )
        if name:
            name = name.strip()
        if not name:
            return
        add_or_update_position(name, x, y)
        self._refresh_list()

    def _selected_name(self):
        sel = self.tree.selection()
        return sel[0] if sel else None

    def _delete_selected(self):
        name = self._selected_name()
        if not name:
            messagebox.showinfo("ยังไม่ได้เลือก", "กรุณาเลือกตำแหน่งจากรายการก่อน")
            return
        if messagebox.askyesno("ยืนยัน", f"ลบตำแหน่ง '{name}' ทิ้งหรือไม่?"):
            delete_position(name)
            self._refresh_list()

    def _use_selected(self):
        name = self._selected_name()
        if not name:
            messagebox.showinfo("ยังไม่ได้เลือก", "กรุณาเลือกตำแหน่งจากรายการก่อน")
            return
        positions = load_positions()
        coord = positions.get(name)
        if coord:
            self.result = (coord["x"], coord["y"])
            self.destroy()


class HotkeyCaptureDialog(tk.Toplevel):
    """
    กล่องโต้ตอบสำหรับ "ตั้งปุ่มลัดเอง" — รอรับการกดปุ่มใด ๆ บนคีย์บอร์ดจริง
    ไม่จำกัดแค่ F1-F12 เหมือนก่อนหน้า รองรับตัวอักษร ตัวเลข และปุ่มพิเศษส่วนใหญ่
    """

    def __init__(self, parent):
        super().__init__(parent)
        self.result = None
        self.title("ตั้งปุ่มลัดเอง")
        self.resizable(False, False)
        self.transient(parent)
        self.grab_set()
        self._listener = None

        frame = ttk.Frame(self, padding=24)
        frame.pack()
        ttk.Label(frame, text="⌨", font=ui_font(28)).pack()
        self.status_var = tk.StringVar(value="กดปุ่มที่ต้องการตั้งเป็นปุ่มลัด...")
        ttk.Label(frame, textvariable=self.status_var, font=ui_font(11)).pack(pady=(6, 14))
        ttk.Button(frame, text="ยกเลิก", command=self._cancel).pack()

        self.protocol("WM_DELETE_WINDOW", self._cancel)
        self.after(150, self._start_listening)

    def _start_listening(self):
        if pynput_keyboard is None:
            self.status_var.set("ฟีเจอร์นี้ต้องติดตั้งตัวช่วยเพิ่มเติมก่อน")
            return

        def on_press(key):
            try:
                if key == pynput_keyboard.Key.esc:
                    # ESC สงวนไว้เป็นปุ่มหยุดฉุกเฉินเสมอ ใช้ตั้งเป็นปุ่มลัดอื่นไม่ได้
                    self.after(0, self._reject_esc)
                    return
                name = getattr(key, "name", None)
                if name:
                    value = name
                else:
                    ch = getattr(key, "char", None)
                    value = ch.lower() if ch else None
                if value:
                    self.after(0, lambda: self._accept(value))
            except Exception:
                pass

        self._listener = pynput_keyboard.Listener(on_press=on_press)
        self._listener.start()

    def _reject_esc(self):
        self.status_var.set("ปุ่ม ESC สงวนไว้แล้ว กรุณากดปุ่มอื่น")

    def _accept(self, value):
        self.result = value
        self._stop_listener()
        self.destroy()

    def _cancel(self):
        self._stop_listener()
        self.destroy()

    def _stop_listener(self):
        if self._listener is not None:
            try:
                self._listener.stop()
            except Exception:
                pass
            self._listener = None


class AboutDialog(tk.Toplevel):
    def __init__(self, parent, version):
        super().__init__(parent)
        self.title("เกี่ยวกับโปรแกรม")
        self.resizable(False, False)
        self.transient(parent)
        self.grab_set()
        frame = ttk.Frame(self, padding=24)
        frame.pack()
        ttk.Label(frame, text="⚡ คลิกอัตโนมัติ", font=ui_font(18, "bold")).pack(pady=(0, 4))
        ttk.Label(frame, text=f"เวอร์ชัน {version}", font=ui_font(10)).pack()
        ttk.Label(frame, text="โปรแกรมช่วยคลิกอัตโนมัติ และสร้างชุดคำสั่งอัตโนมัติของคุณเอง",
                  foreground="gray", font=ui_font(10)).pack(pady=(8, 14))
        ttk.Label(frame,
                  text="ปุ่มลัด: ตั้งค่าได้ในหน้า 'ตั้งค่า'\nปุ่ม ESC = หยุดทุกอย่างทันที",
                  justify="left", font=ui_font(10)).pack(pady=(0, 14))
        ttk.Button(frame, text="ปิด", command=self.destroy).pack()
