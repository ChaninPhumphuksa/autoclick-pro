...
class App:
    ...
    def _build_ui(self):
        ...
        self._section_title(tab1, "🖥  ทำงานเบื้องหลังกับโปรแกรมอื่น (ขั้นสูง)", color=TAB_AUTOCLICK).pack(**pad, anchor="w")
        self.background_mode_var = tk.BooleanVar(value=False)
        bg_check = ttk.Checkbutton(
            tab1, text="เปิดใช้งาน: คลิกที่โปรแกรมที่เลือกไว้ โดยไม่ต้องสลับหน้าต่างมาโฟกัส "
                       "(ไปทำงานโปรแกรมอื่นต่อได้)",
            variable=self.background_mode_var,
        )
        bg_check.pack(padx=12, pady=(0, 4), anchor="w")

        if not windows_target.is_windows():
            bg_check.config(state="disabled")
            ttk.Label(tab1, text="(ฟีเจอร์นี้ใช้ได้เฉพาะเครื่อง Windows เท่านั้น)",
                      font=ui_font(9), foreground="gray").pack(padx=12, anchor="w")
        else:
            ttk.Label(tab1,
            ...
            target_row = ttk.Frame(tab1)
            target_row.pack(padx=12, pady=4, anchor="w", fill="x")
            ...
            target_pos_row = ttk.Frame(tab1)
            target_pos_row.pack(padx=12, pady=4, anchor="w")
            ttk.Label(target_pos_row, text="X", font=ui_font(10)).pack(side="left", padx=(0, 2))
            self.target_x_var = tk.StringVar(value="0")
            ttk.Entry(target_pos_row, textvariable=self.target_x_var, width=6, font=ui_font(10)).pack(side="left")
            ttk.Label(target_pos_row, text="Y", font=ui_font(10)).pack(side="left", padx=(10, 2))
            self.target_y_var = tk.StringVar(value="0")
            ttk.Entry(target_pos_row, textvariable=self.target_y_var, width=6, font=ui_font(10)).pack(side="left")
            ttk.Button(target_pos_row, text="📍 จับตำแหน่งในหน้าต่างเป้าหมาย (3 วิ)",
                       command=self._capture_target_position).pack(side="left", padx=10)
    ...
    def get_click_settings(self):
        ...
        if background_mode:
            ...
            try:
                fx = int(float(self.target_x_var.get()))
                fy = int(float(self.target_y_var.get()))
    ...
    def _refresh_target_windows(self):
        ...
        if display_list and not self.target_window_var.get():
    ...
    def _capture_target_position(self):
        display = self.target_window_var.get()
        if not display or display not in self._window_choices:
            messagebox.showinfo("ยังไม่ได้เลือก", "กรุณาเลือกโปรแกรมเป้าหมายจากรายการก่อน (กด \"รีเฟรชรายการ\" ถ้ายังไม่เห็น)")
            return
        title, process = self._window_choices[display]

        def countdown(n):
            if n > 0:
                self.set_status(f"เตรียมตัว... อีก {n} วินาที (ย้ายเมาส์ไปตำแหน่งในโปรแกรมเป้าหมายที่ต้องการ)")
                self.root.after(1000, lambda: countdown(n - 1))
            else:
                if not pyautogui:
                    return
                ...
                if hwnd is None:
                    ...
                    return
                cx, cy = windows_target.screen_to_client(hwnd, sx, sy)
                self.target_x_var.set(str(cx))
                self.target_y_var.set(str(cy))
                self.set_status(f"จับตำแหน่งในหน้าต่างเป้าหมายแล้ว ✓ ({cx}, {cy})")
        countdown(3)
