...
class App:
    ...
    def _build_ui(self):
        ...
        self.click_toggle_btn.pack(**pad, fill="x", padx=20)
        ...
        self.record_toggle_btn.pack(**pad, fill="x", padx=20)
        ...
        save_row.pack(**pad, fill="x", padx=20)
        ...
        for label, var in [("Auto Click เริ่ม/หยุด:", self.hotkey_click_var),
            ...
            row = ttk.Frame(tab3)
            row.pack(**pad, anchor="w", fill="x", padx=20)
        ...
        ttk.Label(tab3, text="* ปุ่ม ESC ถูกกำหนดตายตัวไว้เป็นปุ่มหยุดฉุกเฉินเสมอ",
                  foreground="gray").pack(**pad, anchor="w", padx=20)
        ...
        startup_row.pack(**pad, anchor="w", padx=20, fill="x")
        ...
        ttk.Checkbutton(tab3, text="ย่อหน้าต่างลงถาด (System Tray) เมื่อกดปิดหน้าต่าง แทนการปิดโปรแกรม",
                         ...
                         command=self._persist_config).pack(**pad, anchor="w", padx=20)
        ...
        if not self.tray.available():
            ttk.Label(tab3, text="* ยังไม่ได้ติดตั้ง pystray/Pillow — ระบบ tray จะไม่ทำงาน (pip install pystray pillow)",
                      foreground="orange").pack(**pad, anchor="w", padx=20)
