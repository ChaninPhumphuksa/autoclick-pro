...
from core.engine import AutoClicker, MacroRecorder, MacroPlayer, MultiMacroRunner, \
    load_macro_file as load_macro_events_from_file, is_directinput_available
...
from core.notify import play_completion_sound
...
class App:
    def __init__(self, root):
        ...
        self.notify_sound_var = tk.BooleanVar(value=self.config.get("play_sound_on_complete", True))
        self.input_backend_var = tk.StringVar(value=self.config.get("input_backend", "pyautogui"))
        self._pending_click_count = 0

        self.clicker = AutoClicker(self.get_click_settings, self.set_status,
                                    on_click_cb=self._on_engine_click,
                                    on_finished_cb=lambda: self._on_engine_finished(is_macro=False))
        ...
        self.player = MacroPlayer(lambda: self.recorder.events, self.get_macro_settings, self.set_status,
                                   on_click_cb=self._on_engine_click,
                                   on_finished_cb=lambda: self._on_engine_finished(is_macro=True))
        self.multi_runner = MultiMacroRunner(self.set_status, on_click_cb=self._on_engine_click)
        ...
        self._build_ui()
        # ล็อกขนาดหน้าต่างอีกครั้งหลังสร้างเนื้อหาทั้งหมดแล้ว เพราะ Tkinter จะขยายหน้าต่าง
        # ให้พอดีกับเนื้อหาที่เพิ่งสร้างเสร็จโดยอัตโนมัติ ถ้าไม่บังคับซ้ำ ขนาดจะไม่ตรง 640x680
    ...
    def _make_scrollable_tab(self, notebook, tab_label):
        """สร้างแท็บที่เลื่อนขึ้น-ลงได้ (เผื่อเนื้อหายาวกว่าหน้าต่าง) คืนค่าเฟรมด้านในให้ใช้แทนแท็บปกติ"""
        ...
        # เลื่อนด้วยลูกกลิ้งเมาส์ได้เฉพาะตอนเมาส์อยู่เหนือแท็บนี้ (ไม่รบกวนแท็บอื่น)
    ...
    def _build_ui(self):
        ...
        tab1 = self._make_scrollable_tab(notebook, "  คลิกอัตโนมัติ  ")
        ...
        self._section_title(tab1, "🎯  ปรับแต่งเพิ่มเติม (ไม่บังคับ)", color=TAB_AUTOCLICK).pack(**pad, anchor="w")
        pos_jitter_row = ttk.Frame(tab1)
        pos_jitter_row.pack(padx=12, pady=4, anchor="w")
        ttk.Label(pos_jitter_row, text="สุ่มตำแหน่งคลิกในรัศมี:", font=ui_font(10)).pack(side="left")
        self.position_jitter_var = tk.StringVar(value="0")
        ttk.Entry(pos_jitter_row, textvariable=self.position_jitter_var, width=6, font=ui_font(10)).pack(
            side="left", padx=6)
        ttk.Label(pos_jitter_row, text="พิกเซล (ทำให้จุดคลิกไม่ตรงเป๊ะทุกครั้ง)", font=ui_font(9),
                  foreground="gray").pack(side="left")

        start_delay_row = ttk.Frame(tab1)
        start_delay_row.pack(padx=12, pady=4, anchor="w")
        ttk.Label(start_delay_row, text="หน่วงเวลาก่อนเริ่มทำงาน:", font=ui_font(10)).pack(side="left")
        self.start_delay_var = tk.StringVar(value="0")
        ttk.Entry(start_delay_row, textvariable=self.start_delay_var, width=6, font=ui_font(10)).pack(
            side="left", padx=6)
        ttk.Label(start_delay_row, text="วินาที (นับถอยหลังหลังกดเริ่ม เผื่อเวลาสลับหน้าต่าง)", font=ui_font(9),
                  foreground="gray").pack(side="left")
        ...
        tab2 = self._make_scrollable_tab(notebook, "  ชุดคำสั่ง  ")
        ...
        step_btns_extra = ttk.Frame(tab2)
        step_btns_extra.pack(padx=24, pady=2, fill="x")
        ttk.Button(step_btns_extra, text="+ พิมพ์ข้อความ",
                   command=lambda: self._add_step_dialog("type")).pack(side="left", padx=3)
        ttk.Button(step_btns_extra, text="+ รอสีเปลี่ยน",
                   command=lambda: self._add_step_dialog("wait_pixel")).pack(side="left", padx=3)
        ttk.Button(step_btns_extra, text="+ เริ่มวนซ้ำ",
                   command=lambda: self._add_step_dialog("loop_start")).pack(side="left", padx=3)
        ttk.Button(step_btns_extra, text="+ จบวนซ้ำ",
                   command=lambda: self._add_step_dialog("loop_end")).pack(side="left", padx=3)
        ttk.Button(step_btns_extra, text="ทำซ้ำขั้นตอนนี้",
                   command=self._duplicate_selected_step).pack(side="left", padx=3)
        ...
        ttk.Label(step_btns2, text="(คลิกขวาที่รายการเพื่อเปิดเมนูลัด)", font=ui_font(9),
        ...
        search_row.pack(padx=24, pady=(0, 4), fill="x")
        ttk.Label(search_row, text="🔍 ค้นหา:", font=ui_font(9)).pack(side="left")
        self.library_search_var = tk.StringVar()
        self.library_search_var.trace_add("write", lambda *a: self._refresh_library_list())
        ttk.Entry(search_row, textvariable=self.library_search_var, width=24, font=ui_font(9)).pack(
            side="left", padx=6)
        ...
        ttk.Button(lib_row, text="เปลี่ยนชื่อ", command=self._rename_library_item).pack(side="left", padx=3)
        ...
        # ================= หน้า 3: ตั้งค่า =================
        tab3 = self._make_scrollable_tab(notebook, "  ตั้งค่า  ")
        ...
        self._section_title(tab3, "🎨  รูปลักษณ์และการแจ้งเตือน", color=TAB_SETTINGS).pack(**pad, anchor="w")
        theme_row = ttk.Frame(tab3)
        theme_row.pack(padx=24, pady=(4, 4), anchor="w")
        ttk.Label(theme_row, text="ธีมสี:", font=ui_font(10)).pack(side="left")
        ttk.Radiobutton(theme_row, text="มืด", value="dark", variable=self.theme_var,
                         command=self._on_change_theme).pack(side="left", padx=(8, 0))
        ttk.Radiobutton(theme_row, text="สว่าง", value="light", variable=self.theme_var,
                         command=self._on_change_theme).pack(side="left", padx=(8, 0))
        ttk.Checkbutton(tab3, text="🔔 เล่นเสียงแจ้งเตือนเมื่อทำงานเสร็จตามที่ตั้งไว้ (ไม่เล่นตอนกดหยุดเอง)",
                         variable=self.notify_sound_var, command=self._persist_config).pack(
            padx=24, pady=(2, 4), anchor="w")

        backend_row = ttk.Frame(tab3)
        backend_row.pack(padx=24, pady=(4, 2), anchor="w")
        ttk.Label(backend_row, text="วิธีคลิก/ลาก:", font=ui_font(10)).pack(side="left")
        ttk.Radiobutton(backend_row, text="ปกติ", value="pyautogui",
                         variable=self.input_backend_var, command=self._persist_config).pack(side="left", padx=(8, 0))
        directinput_radio = ttk.Radiobutton(backend_row, text="โหมดเข้ากันได้กับเกม (DirectInput)",
                                             value="directinput", variable=self.input_backend_var,
                                             command=self._persist_config)
        directinput_radio.pack(side="left", padx=(8, 0))
        if not is_directinput_available():
            directinput_radio.config(state="disabled")
            ttk.Label(tab3, text="* ยังไม่ได้ติดตั้งตัวช่วยโหมดนี้ (pip install pydirectinput-rgx) — ใช้แล้วจะ "
                                  "คลิกติดกับเกมที่ปกติคลิกไม่เข้า เช่นเกมที่อ่านค่าเมาส์ผ่าน DirectInput",
                      font=ui_font(9), foreground="#F59E0B", wraplength=540, justify="left").pack(
                padx=24, pady=(0, 4), anchor="w")
        else:
            ttk.Label(tab3, text="ใช้โหมดนี้ถ้าคลิกปกติแล้วเกมไม่ตอบสนอง (ใช้ได้เฉพาะตอนไม่ได้เปิดทำงานเบื้องหลัง)",
                      font=ui_font(9), foreground="gray", wraplength=540, justify="left").pack(
                padx=24, pady=(0, 4), anchor="w")
        ...
        ttk.Separator(tab3).pack(fill="x", padx=12, pady=14)
        ttk.Separator(tab3).pack(fill="x", padx=12, pady=14)
        self._section_title(tab3, "📊  สถิติการใช้งาน", color=TAB_SETTINGS).pack(**pad, anchor="w")
        self.stats_label_var = tk.StringVar(value="")
        ttk.Label(tab3, textvariable=self.stats_label_var, font=ui_font(10), justify="left").pack(
            padx=24, pady=(2, 4), anchor="w")
        ttk.Button(tab3, text="รีเซ็ตสถิติ", command=self._reset_stats).pack(padx=24, pady=(0, 4), anchor="e")
    ...
    def _on_engine_click(self):
        # เรียกจากเธรดพื้นหลัง แค่นับเก็บไว้ก่อน แล้วค่อยบันทึกลงไฟล์เป็นชุด (กัน I/O ถี่เกินไป)
        self._pending_click_count += 1

    def _on_engine_finished(self, is_macro):
        # เรียกเฉพาะตอน "ทำงานจนครบตามที่ตั้งไว้เอง" ไม่เรียกตอนผู้ใช้กดหยุดเอง
        if is_macro:
            stats_module.record_macro_run()
        if getattr(self, "notify_sound_var", None) and self.notify_sound_var.get():
    ...
    def _on_change_theme(self):
        apply_theme(self.root, mode=self.theme_var.get())
        self._persist_config()
        self.set_status(f"เปลี่ยนธีมเป็น: {'มืด' if self.theme_var.get() == 'dark' else 'สว่าง'}")

    def _refresh_stats_display(self):
        s = stats_module.get_stats()
        self.stats_label_var.set(
            f"คลิกไปทั้งหมด: {s['total_clicks']:,} ครั้ง   (วันนี้ {s['today_clicks']:,} ครั้ง)\n"
            f"เล่นชุดคำสั่งจนจบแล้ว: {s['total_macro_runs']:,} ครั้ง"
        )

    def _reset_stats(self):
        if messagebox.askyesno("ยืนยัน", "ล้างสถิติการใช้งานทั้งหมดหรือไม่? (ทำแล้วย้อนกลับไม่ได้)"):
            stats_module.reset_stats()
            self._refresh_stats_display()
            self.set_status("ล้างสถิติการใช้งานแล้ว")

    def _flush_pending_stats(self):
        if self._pending_click_count > 0:
            n, self._pending_click_count = self._pending_click_count, 0
            stats_module.record_click(n)
            self._refresh_stats_display()
        self.root.after(1000, self._flush_pending_stats)
    ...
    def _persist_config(self):
        ...
        self.config.update({
            ...
            "play_sound_on_complete": self.notify_sound_var.get(),
            "input_backend": self.input_backend_var.get(),
        })
    ...
    def get_click_settings(self):
        ...
        try:
            ...
            jitter_seconds = 0.0
        try:
            position_jitter_pixels = max(0, int(float(self.position_jitter_var.get() or 0)))
        except ValueError:
            position_jitter_pixels = 0
        try:
            start_delay_seconds = max(0.0, float(self.start_delay_var.get() or 0))
        except ValueError:
        ...
        return {
            ...
            "position_jitter_pixels": position_jitter_pixels, "start_delay_seconds": start_delay_seconds,
        ...
        def _update():
            ...
            for i, ev in enumerate(self.recorder.events):
                if ev["type"] == "click":
                    ...
                    mods = "+".join(m.capitalize() for m in ev.get("modifiers", []))
                    detail = f"กดปุ่ม: {mods + '+' if mods else ''}{ev['key']}"
                    type_label = "กดปุ่ม"
                elif ev["type"] == "type":
                    preview = ev.get("text", "")
                    if len(preview) > 30:
                        preview = preview[:30] + "…"
                    detail = f'พิมพ์: "{preview}"'
                    type_label = "พิมพ์ข้อความ"
                elif ev["type"] == "wait_pixel":
                    r, g, b = ev.get("color", [0, 0, 0])
                    cond = "เป็น" if ev.get("match", True) else "ไม่เป็น"
                    detail = f"({ev.get('x',0)}, {ev.get('y',0)}) รอจนสี{cond} #{r:02x}{g:02x}{b:02x} (สูงสุด {ev.get('timeout',5)} วิ)"
                    type_label = "รอสีเปลี่ยน"
                elif ev["type"] == "loop_start":
                    detail = f"วนซ้ำ {ev.get('repeat', 1)} รอบ"
                    type_label = "🔁 เริ่มวนซ้ำ"
                elif ev["type"] == "loop_end":
                    detail = "-"
                    type_label = "🔁 จบวนซ้ำ"
                else:
                    detail = "-"
    ...
    def _duplicate_selected_step(self):
        idx = self._selected_step_index()
        if idx is None:
            messagebox.showinfo("ยังไม่ได้เลือก", "กรุณาเลือกขั้นตอนที่ต้องการทำซ้ำจากรายการก่อน")
            return
        self.recorder.duplicate_step(idx)
        self.step_tree.selection_set(str(idx + 1))
        self.set_status("ทำซ้ำขั้นตอนแล้ว")
    ...
    def _show_step_context_menu(self, event):
        ...
        menu.add_command(label="แก้ไข", command=self._edit_selected_step)
        menu.add_command(label="ทำซ้ำขั้นตอนนี้", command=self._duplicate_selected_step)
        menu.add_command(label="ลบ", command=self._remove_selected_step)
        menu.add_separator()
        menu.add_command(label="▲ เลื่อนขึ้น", command=lambda: self._move_selected_step(-1))
        menu.add_command(label="▼ เลื่อนลง", command=lambda: self._move_selected_step(1))
    ...
    def _refresh_library_list(self):
        all_files = sorted(f[:-5] for f in os.listdir(MACRO_LIBRARY_DIR) if f.endswith(".json"))
        query = self.library_search_var.get().strip().lower() if hasattr(self, "library_search_var") else ""
        files = [f for f in all_files if query in f.lower()] if query else all_files
        self.library_combo["values"] = files
        if files and self.library_var.get() not in files:
    ...
    def _save_to_library(self):
        ...
        if os.path.exists(path):
            if not messagebox.askyesno("มีชื่อนี้อยู่แล้ว",
                                        f"มีชุดคำสั่งชื่อ '{safe_name}' อยู่แล้ว ต้องการบันทึกทับหรือไม่?"):
                return
    ...
    def _rename_library_item(self):
        old_name = self.library_var.get()
        if not old_name:
            messagebox.showinfo("ยังไม่ได้เลือก", "กรุณาเลือกชุดคำสั่งที่ต้องการเปลี่ยนชื่อก่อน")
            return
        new_name = simpledialog.askstring("เปลี่ยนชื่อชุดคำสั่ง", f"ตั้งชื่อใหม่แทน '{old_name}':",
                                           initialvalue=old_name, parent=self.root)
        if not new_name:
            return
        safe_new_name = "".join(c for c in new_name.strip() if c.isalnum() or c in " _-").strip()
        if not safe_new_name or safe_new_name == old_name:
            return
        old_path = os.path.join(MACRO_LIBRARY_DIR, f"{old_name}.json")
        new_path = os.path.join(MACRO_LIBRARY_DIR, f"{safe_new_name}.json")
        if os.path.exists(new_path):
            messagebox.showerror("เปลี่ยนชื่อไม่สำเร็จ", f"มีชุดคำสั่งชื่อ '{safe_new_name}' อยู่แล้ว")
            return
        try:
            os.rename(old_path, new_path)
            self._refresh_library_list()
            self.library_var.set(safe_new_name)
            self.set_status(f"เปลี่ยนชื่อเป็น '{safe_new_name}' แล้ว")
        except Exception as e:
            messagebox.showerror("เปลี่ยนชื่อไม่สำเร็จ", str(e))
